...
# Baptiste M <baptistem@laposte.net> ( un peu adapte pour solidworks )
...
class OpenPLMPluginInstance(object):
    ...
    PLUGIN_DIR = os.path.join(OPENPLM_DIR, "SWCAD")
    ...
    def save_as_step_gif(self):
        ...
        if ( swModel is None):
            ...
            nom_fichier_stp = nom_fichier[:-5]+".step"
            print(nom_fichier,nom_fichier_stp)
            print(swModel.SaveAs(nom_fichier_stp))
            nom_fichier_gif = nom_fichier[:-5]+".gif"
            print(nom_fichier,nom_fichier_gif)
            print(swModel.SaveAs(nom_fichier_gif))
            return [nom_fichier_stp,nom_fichier_gif]
    ...
    def create(self, data, filename, unlock):
        ...
        if res["result"] != "ok":
            ...
            nom_fichier_stp = os.path.join(rep,(fileName+".step")).encode("utf-8")
            print(swModel.SaveAs(nom_fichier_stp),nom_fichier_stp)
            nom_fichier_png = os.path.join(rep,(fileName+".jpg")).encode("utf-8")
            print(swModel.SaveAs(nom_fichier_png),nom_fichier_png)

            #upload stp and SWCAD object
            doc_step_file=self.upload_file(doc, nom_fichier_stp.encode("utf-8"))
    ...
    def get_data(self, url, data=None, show_errors=True, reraise=False):
        ...
        try:
            e = json.load(self.opener.open(self.SERVER + url, data_enc))
            print("GET DATA : RETOUR",e)
            return e
    ...
    def send_thumbnail(self, gdoc):
        doc = self.documents[gdoc]["openplm_doc"]
        doc_file_id = self.documents[gdoc]["openplm_file_id"]
        print("THUMBMAIL GDOC",gdoc)
        [stp_filename,jpg_filename] = self.save_as_step_gif()
        
        datagen, headers = multipart_encode({"filename": open(jpg_filename, "rb")})
        # Create the Request object
...
class SearchDialog(Dialog):
    ...
    def action_cb(self):
        ...
        if not node.parent():
            return
        print("ACTION_CB",node)
        print("ACTION_CB",node.parent())
...
SWCADGui.addCommand('OpenPLM_CheckOut', OpenPLMCheckOut())
...
SWCADGui.addCommand('OpenPLM_Download', OpenPLMDownload())
...
SWCADGui.addCommand('OpenPLM_Forget', OpenPLMForget())
...
SWCADGui.addCommand('OpenPLM_CheckIn', OpenPLMCheckIn())
...
SWCADGui.addCommand('OpenPLM_Revise', OpenPLMRevise())
...
SWCADGui.addCommand('OpenPLM_AttachToPart', OpenPLMAttach())
...
SWCADGui.addCommand('OpenPLM_Create', OpenPLMCreate())
