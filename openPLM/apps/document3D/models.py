...
class Document3D(pmodels.Document):
    ...
    PartDecompose = models.ForeignKey(pmodels.Part,null=True,on_delete=models.CASCADE)
...
media_3d_geometry_path = Path(settings.MEDIA_ROOT) / "3D"
media3DGeometryFile = pmodels.DocumentStorage(location=str(media_3d_geometry_path))
class GeometryFile(models.Model):
    ...
    stp = models.ForeignKey(pmodels.DocumentFile,on_delete=models.CASCADE)
...
media3DArbreFile_path = Path(settings.MEDIA_ROOT)/"3D/"
media3DArbreFile = pmodels.DocumentStorage(location=str(media3DArbreFile_path))
...
def update_root_BD(new_stp_file,stp_file,ctrl,product,file,name,part):
    ...
    new_stp_file.save()
    os.chmod(new_stp_file.file.path, Oo400)
