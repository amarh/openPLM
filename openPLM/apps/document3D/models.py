...
class Document3DController(DocumentController):
    ...
    def handle_added_file(self, doc_file):
        ...
        if fileExtension.upper() in ('.STP', '.STEP'):
            ...
            try:
                #handle_step_file.delay(doc_file.pk)#It generates entitys ArbreFile and Geometry and associates this classes to the documentFile
                handle_step_file.delay(doc_file.pk)
            except Exception as excep:
                
                if type(excep)==TimeoutError:
                    #TimeoutError SEND MAIL? 
                    raise excep
                else:
                    #Indeterminate error SEND MAIL?
                    raise excep    
...
class Document3D_generate_Error(Exception):
    def __unicode__(self):
        return u"Error during the generation of the ArbreFile and GeometryFiles"        
class Document3D_generate_Index_Error(Exception):
    def __unicode__(self):
        return u"Error , try to assign an index not validate for a geometry"               
...
class Document_Generate_Bom_Error(Exception):
...
def add_child_GeometryFiles(product,files_to_add):
    ...
    """
    for link in product.links:
        if not link.product.doc_id==product.doc_id and link.product.visited==False:
