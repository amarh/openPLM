...
"""
    ...
    def get_js(self):
        if not self.product:
            return ""
        self.menu_items.append(self._get_menu_item(0, self.product.name))
...
object%s.matrix.multiplySelf(new THREE.Matrix4(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,0,0,0,1));\n"""
    ...
    def _get_menu_item(self, counter,name):

        onclick="change_part"+str(counter)+"(\\\"click\\\")" 
        return "<li > <a href='#' id='li-part-%s' onClick='%s'><b onClick='%s'></b>%s </a>" % (counter, onclick,onclick,name)
