...
class display_3dTest(CommonViewTest):

    def setUp(self):
        super(display_3dTest, self).setUp()
    ...
    def test_3D_form_post(self):
        ...
        response_post = self.post(self.document.object.plmobject_url+"3D/", 
                            data)     
        self.assertEqual(5, len(list(response_post.context["file_geometry_js"])))
    ...
    def test_3D_form_post_no_arborescense(self):
        ...
        new_doc_file=self.document.add_file(myfile)          
        form=Form3D(document=self.document)
        arbrefile_documentFile_stp.objects.get(stp=new_doc_file).delete()
    ...
    def test_3D_no_stp_associe(self):   
        ...
        response = self.get(self.document.object.plmobject_url+"3D/")
        self.assertEqual(response.context["file_geometry_js"], [])
    ...
    def test_3D_stp_associe_sans_arborescense(self):   
        ...
        arbrefile_documentFile_stp.objects.get(stp=new_doc_file).delete()
        response = self.get(self.document.object.plmobject_url+"3D/")    
        self.assertEqual(3, len(list(response.context["file_geometry_js"])))
        self.assertEqual(response.context["javascript_arborescense"], False)
    ...
    def test_3D_stp_associe_sans_geometry_with_arborescense(self):   
        ...
        myfile = File(f)
        new_doc_file=self.document.add_file(myfile)       
        stp_to_jss.objects.filter(stp=new_doc_file).delete()
        response = self.get(self.document.object.plmobject_url+"3D/")    
        self.assertEqual([], response.context["file_geometry_js"])
        self.assertNotEqual(response.context["javascript_arborescense"], False)  
       
class display_bomb_childTest(CommonViewTest):
    def setUp(self):
        super(display_bomb_childTest, self).setUp()
        self.document = Document3DController.create('doc1', 'Document3D',
                'a', self.user, self.DATA)
...
class display_decomposeTest(CommonViewTest):

    def setUp(self):
        super(display_decomposeTest, self).setUp()
        self.document = Document3DController.create('doc1', 'Document3D',
                'a', self.user, self.DATA)
    ...
    def test_display_decompose_Document_part_doc_links_Error(self):
        ...
        self.assertEqual(reponse.context["extra_errors"],"Columns reference, type, revision are not unique")
    ...
    def test_display_decompose_Document3D_decomposer_Error(self):
        ...
        stp_to_jss.objects.filter(stp=new_doc_file).delete()
        reponse=self.post(self.base_url+"decompose/"+str(new_doc_file.id)+"/",data)
        self.assertEqual(reponse.context["extra_errors"],"Error while the file step was decomposed")        
