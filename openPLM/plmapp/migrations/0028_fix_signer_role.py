...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for model in ["plmapp.DelegationLink", "plmapp.PLMObjectUserLink"]:
        ...
        orm["plmapp.History"].objects.filter(action="New sign_3rd_level").update(action="New sign_4th_level")
        orm["plmapp.History"].objects.filter(action="New sign_3th_level").update(action="New sign_3rd_level")
        orm["plmapp.History"].objects.filter(action="sign_3rd_level removed").update(action="sign_4th_level removed")
        orm["plmapp.History"].objects.filter(action="sign_3th_level removed").update(action="sign_3rd_level removed")
    ...
    def backwards(self, orm):
        ...
        orm["plmapp.History"].objects.filter(action="New sign_3rd_level").update(action="New sign_3th_level")
        orm["plmapp.History"].objects.filter(action="New sign_4th_level").update(action="New sign_3rd_level")
        orm["plmapp.History"].objects.filter(action="sign_3rd_level removed").update(action="sign_3th_level removed")
        orm["plmapp.History"].objects.filter(action="sign_4th_level removed").update(action="sign_3rd_level removed")
    ...
    models = {
        ...
        'plmapp.invitation': {
            ...
            'token': ('django.db.models.fields.CharField', [], {'default': "'1902249160190237751229351738032482314211581663428213657105606785065011947997981865052810166099828993117285773572738361674515709362122570512362855478191535'", 'max_length': '155', 'primary_key': 'True'}),
        ...
        'plmapp.statehistory': {
            ...
            'lifecycle': ('django.db.models.fields.related.ForeignKey', [], {'to': "orm['plmapp.Lifecycle']"}),
            'plmobject': ('django.db.models.fields.related.ForeignKey', [], {'to': "orm['plmapp.PLMObject']"}),
            'start_time': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2013, 1, 15, 0, 0)'}),
            'state': ('django.db.models.fields.related.ForeignKey', [], {'to': "orm['plmapp.State']"}),
