...
def browse(request, type="object"):
    ...
    if user.is_authenticated() and not user.get_profile().restricted:
        ...
        if type in ("object", "part", "topassembly", "document"):
            ctx["plmobjects"] = True
            object_list = object_list.exclude(lifecycle=models.get_cancelled_lifecycle())
            if state == "official":
                object_list = object_list.filter(state=F("lifecycle__official_state"))
        ...
        object_list = cls.filter(query).exclude(lifecycle=models.get_cancelled_lifecycle())
